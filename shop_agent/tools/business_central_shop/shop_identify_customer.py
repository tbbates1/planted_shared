"""Tool: identify a customer by name or email for the shop agent."""
...
@tool(
    ...
    description="Identify a customer by name or email. Returns customer_id, business name, last shipped order, and pending orders. Call this FIRST before any other tool.",
)
def shop_identify_customer(context: AgentRun, search_term: str) -> dict:
    """Look up a customer in Business Central by name or email.

    Args:
        context: Agent run context (auto-filled).
        search_term: Customer name or email to search for (e.g. "Migros" or "orders@migros.ch").

    Returns:
        dict: Keys: customer_id, business_name, last_shipped, pending_orders.
    """
    if not search_term or not search_term.strip():
        return {"error": "search_term is required. Ask the customer for their company name or email."}
    ...
    # Match by email first, then by name (case-insensitive contains)
    customer = None
    for c in customers:
        bc_email = (c.get("email") or "").strip().lower()
        if bc_email and bc_email == search_term:
            customer = c
            break

    if not customer:
        for c in customers:
            bc_name = (c.get("displayName") or "").strip().lower()
            if search_term in bc_name or bc_name in search_term:
                customer = c
                break

    if not customer:
        return {
            "error": f"No customer found matching '{search_term}'. "
