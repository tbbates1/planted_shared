"""Tool: cancel (delete) a pending sales quote by reference number."""
...
@tool(
    ...
    description="Cancel a pending order by reference number (SQ####). Deletes the sales quote from Business Central. Shipped orders (SO) cannot be cancelled.",
)
def wa_cancel_order(context: AgentRun, customer_id: str = "", quote_number: str = "", reference_number: str = "") -> dict:
    ...
    Args:
        ...
        reference_number: Alias for quote_number (backward compat).
    """
    # Accept either parameter name (old code used reference_number)
    quote_number = quote_number or reference_number
    if not quote_number:
        return {"error": "quote_number is required."}

    # If customer_id not passed by LLM, try reading from context variable
    ...
    del_resp = requests.delete(f"{base}/companies({COMPANY_ID})/salesQuotes({quote['id']})", headers=del_headers, timeout=30)
    if not del_resp.ok:
