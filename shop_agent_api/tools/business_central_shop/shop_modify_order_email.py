"""Tool: modify an existing pending order (sales quote) in Business Central (email shop agent)."""
...
@tool(
    ...
    name="shop_modify_order_email",
    description="Modify an existing pending order by reference number. Replaces ALL items on the order — include every item the order should have after changes. Customer is resolved automatically from email_address context variable.",
)
def shop_modify_order_email(
    context: AgentRun,
