"""Tool: cancel (delete) a pending sales quote by reference number (WhatsApp shop agent)."""
...
@tool(
    ...
    name="shop_cancel_order_wa",
    description="Cancel a pending order by reference number (SQ####). Shipped orders (SO) cannot be cancelled. Returns the cancelled order details. Customer is resolved automatically from phone_number context variable.",
)
def shop_cancel_order_wa(context: AgentRun, quote_number: str) -> dict:
